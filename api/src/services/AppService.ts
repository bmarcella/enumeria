...
import { ChatAi, Service_tier } from '../entity/ChatAi';
...
const services = {
  ...
  chat: async (req: Request, res: Response) => {
    ...
    const completion = await req.AI.chat.completions.create({
     ...
     let chat : any;
     if(isNew) {
          chat =  await  DSave<ChatAi>(req.DB, ChatAi, {
          id: uuidv4(),
  ...
  setSummary: async (req: Request, data: ChatAi, completion: ChatCompletion & {
    ...
    return new Promise(async (resolve) => {
      ...
      const ai =  await  DSave<MessageAi>(req.DB, MessageAi, {
        chatAi: data,
        role: 'system',
      ...
      data = await  DSave<ChatAi>(req.DB, ChatAi, data);
      resolve({ data, ai  } );
    });
  },
  getSummary: async (req: Request) => {
    return new Promise(async (resolve) => {
     const chat = await DGet<ChatAi>(req.DB, ChatAi, false,{ where : { id: req.body.chat.id } } ) as ChatAi;
     const message = await  DSave<MessageAi>(req.DB, MessageAi, {
      chatAi: chat ,
      role: req.body.role,
