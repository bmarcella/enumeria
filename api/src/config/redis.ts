...
function makeRedisConnection(name: string): Redis | undefined {
  try {
    const conn = redisUrl
      ? new IORedis(redisUrl, buildRedisOptions())
      : new IORedis(buildRedisOptions());
    if (conn) attachRedisListeners(conn, name);
    return conn;
  } catch (error) {
...
async function tryConnect(conn: Redis | undefined, name: string): Promise<void> {
  if (!conn) return;
  try {
    await conn.connect();
  } catch {
    console.warn(
      `\n⚠  [Redis ${name}] Redis is unavailable — the app will continue without Redis.` +
      `\n   Queues, caching, and real-time features that depend on Redis will not work.` +
      `\n   Redis will auto-reconnect when it becomes available.\n`,
    );
  }
}
...
// Attempt to connect but don't block or crash if Redis is down
void tryConnect(connection, 'default');
 ...
 */
export function createRedisConnection(name = 'dedicated'): Redis | undefined {
  const conn = makeRedisConnection(name);
  void tryConnect(conn, name);
  return conn;
}
