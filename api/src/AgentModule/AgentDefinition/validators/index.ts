...
export const AgentManifestSchema = z
  ...
  .superRefine((m, ctx) => {
    // 1) router config required when entry.kind = router
    ...
    // 4) unique runnable lambda alias ids
    ...
    // 5) router entry routes must target existing subAgent ids
    if (m.entry.kind === 'router' && m.entry.router) {
    ...
    // 6) subAgent.tools must exist in tool registry
    ...
    // 7) pipeline-specific validations
...
// 1) Base (ZodObject pur) => .partial() possible
