            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">
                        Create reusable custom tools (code, container, or wasm)
                        to plug into agent manifests.
                    </p>
                </div>
                <div className="flex gap-2">
                    <Button
                        variant="default"
                        className="flex items-center gap-2 shadow-sm rounded-sm"
                        onClick={() => fetchList()}
                    >
                        Refresh
                    </Button>
            ...
            <div>
                <div className="flex items-center gap-2 mb-6">
                    <Input
                        ...
                        placeholder="Search by name, runtime, version..."
                    />
                </div>

                {error && <p className="text-sm text-red-500 mt-3">{error}</p>}
                {loading ? (
                    <p className="text-sm text-slate-500 mt-3 p-4">
                        Loading...
                    </p>
                ) : filtered.length === 0 ? (
                    <div className="border border-dashed border-slate-300 rounded-sm p-12 flex flex-col items-center text-center justify-center bg-slate-50/50 mt-4">
                        ...
                        <p className="text-sm text-slate-500 max-w-sm mb-4">
                            You haven't created any tool artifacts yet. Get
                            started by building your first one.
                        </p>
                        <Button
                            variant="default"
                            className="flex items-center gap-2 rounded-sm shadow-sm"
                            onClick={gotToCreate}
                        >
                            <HiPlus className="w-4 h-4" />
...
const ToolArtifactsListPage = (props: { gotToCreate?: () => void }) => {
    return (
        <ToolArtifactsListPageBase
            gotToCreate={props.gotToCreate}
        ></ToolArtifactsListPageBase>
    )
}
