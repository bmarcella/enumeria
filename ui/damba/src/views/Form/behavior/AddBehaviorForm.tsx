...
import { Form, FormItem, Input, Button, Select, Checkbox, Card } from '@/components/ui'
import { BehaviorFormSchema } from './BehaviorFormSchema'

const HttpEnum = z.enum(['GET', 'POST', 'PUT', 'DELETE', 'PATCH'])
    ...
    <Card className="p-4 rounded-2xl shadow-sm">
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <div className="text-xs opacity-60">Define the route + attach middlewares + extras</div>
        </div>
      </div>
      ...
      <Form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <FormItem label="Method" invalid={!!errors.method} errorMessage={errors.method?.message}>
            <Controller
              name="method"
              control={control}
              render={({ field }) => (
                <Select
                  ...
                  onChange={(opt: any) => field.onChange(opt?.value as Http)}
                />
              )}
            />
          </FormItem>

          <FormItem label="Path" invalid={!!errors.path} errorMessage={errors.path?.message}>
            <Controller
              name="path"
              control={control}
              render={({ field }) => (
          ...
          <Controller
            ...
            render={({ field }) => (
              <Select
                ...
                value={middlewareOptions.filter((o) => (field.value ?? []).includes(o.value))}
                onChange={(opts: any[]) => field.onChange((opts ?? []).map((o) => o.value))}
              />
            )}
          />
          <div className="text-xs opacity-60 mt-1">
            These should match existing middleware records (Many-to-Many).
          </div>
        </FormItem>
        ...
        <div className="rounded-xl border p-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-xs opacity-60">Saved as child records (One-to-Many)</div>
            </div>
            <Button
              type="button"
              variant="default"
          ...
          {fields.length === 0 ? (
            <div className="text-sm opacity-60 mt-3">No extras yet.</div>
          ) : (
              ...
              {fields.map((f, idx) => (
                <div key={f.id} className="grid grid-cols-12 gap-2 items-center">
                  <div className="col-span-5">
                    <FormItem
                      label={idx === 0 ? 'Name' : ''}
                      invalid={!!errors.extras?.[idx]?.name}
                      errorMessage={errors.extras?.[idx]?.name?.message}
                    >
                      <Controller
                        name={`extras.${idx}.name`}
                        control={control}
                        render={({ field }) => <Input {...field} placeholder="e.g. requireAuth" />}
                      />
                    </FormItem>
                  </div>
                  ...
                  <div className="col-span-6">
                    <FormItem
                      label={idx === 0 ? 'Value' : ''}
                      invalid={!!errors.extras?.[idx]?.value}
                      errorMessage={errors.extras?.[idx]?.value?.message as any}
                    >
                      <Controller
                        name={`extras.${idx}.value`}
                        control={control}
                        render={({ field }) => <Input {...field} placeholder="e.g. true" />}
                      />
                    </FormItem>
                  </div>
