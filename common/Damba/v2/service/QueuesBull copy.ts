...
export function agentRunQueueName(
  baseName: string,
  tenantId: string,
  shardCount = 128
) {
...
function makeKey(keyPrefix: string, name: string) {
  return `${keyPrefix ? keyPrefix + ":" : ""}${name}`;
}
...
export function getQueue<TQueue>(
  ...
  isQueuePrefixEnabled: boolean = false,
  keyPrefix?: string
): TQueue {
  assertCtor(QueueClass, "QueueClass");

  const reg = getRegistry();
  const prefix = isQueuePrefixEnabled
    ? keyPrefix ?? getKeyPrefix("default")
    : undefined;
...
export function getQueueEvents<TQueueEvents = any>(
  ...
  const reg = getRegistry();
  const prefix = isQueuePrefixEnabled
    ? ev?.prefix ?? getKeyPrefix("default")
    : undefined;
  const key = makeKey(prefix ?? "", name);
  ...
  if (onCompleted) {
    const lk = `${prefix ? prefix + ":" : ""}${name}:completed`;
  ...
  if (onFailed) {
    const lk = `${prefix ? prefix + ":" : ""}${name}:failed`;
    if (!reg.queueListenerRegistry.has(lk)) {
      reg.queueListenerRegistry.add(lk);
  ...
  if (onProgress) {
    const lk = `${prefix ? prefix + ":" : ""}${name}:progress`;
    if (!reg.queueListenerRegistry.has(lk)) {
      reg.queueListenerRegistry.add(lk);
