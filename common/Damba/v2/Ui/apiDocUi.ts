...
import { NestedApiDoc, RouteDocEntry } from "../route/DambaRouteDoc";
...
function extractZodFields(schema: any): FieldInfo[] | null {
  ...
  // ZodEffects (refine/transform) — unwrap inner
...
function resolveZodType(field: any): string {
  ...
  switch (typeName) {
    ...
    case "ZodLiteral":
      return `"${def.value}"`;
    case "ZodArray":
      return `${resolveZodType(def.type)}[]`;
    case "ZodOptional":
      return resolveZodType(def.innerType);
    case "ZodNullable":
      return `${resolveZodType(def.innerType)} | null`;
    case "ZodDefault":
      return resolveZodType(def.innerType);
    case "ZodObject":
      return "object";
    case "ZodRecord":
      return "record";
    case "ZodUnion":
      return (def.options ?? []).map((o: any) => resolveZodType(o)).join(" | ");
    case "ZodUndefined":
      return "undefined";
    case "ZodAny":
      return "any";
    case "ZodEffects":
      return resolveZodType(def.schema);
    default:
...
const renderSchemaTable = (label: string, colorClass: string, fields: FieldInfo[]) => {
  ...
  const rows = fields
    .map((f) => {
      ...
      return `<tr>
        ...
        <td><code class="field-type">${esc(f.type)}</code></td>
        <td>${req} ${desc}</td>
      </tr>`;
    })
    .join("");
  ...
  return `
    <div class="schema-section">
      <div class="schema-label ${colorClass}">${label}</div>
      <table class="schema-table">
        <thead><tr><th>Field</th><th>Type</th><th>Info</th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </div>`;
};

const renderRoute = (route: RouteDocEntry, theme: DambaTheme) => {
  const mwTotal = route.serviceMiddlewareCount + route.routeMiddlewareCount;
  const timeout = route.timeout
    ? `<span class="tag">timeout: ${route.timeout.in ?? route.timeout.out ?? "?"}ms</span>`
    : "";
  const mwTag = mwTotal > 0 ? `<span class="tag">${mwTotal} middleware</span>` : "";
  const desc = route.decription
    ? `<p class="route-desc">${esc(route.decription)}</p>`
    : "";
  ...
  return `
    <div class="route-row">
      <div class="route-main">
        ...
        <code class="route-path">${esc(route.fullPath)}</code>
        ${expandBtn}
      </div>
      ${desc}
...
const renderService = (
  mount: string,
  methods: Record<string, Record<string, RouteDocEntry>>,
  theme: DambaTheme,
) => {
  let routeCount = 0;
  let routesHtml = "";
    ...
    <div class="service-card">
      <div class="service-header" onclick="this.parentElement.classList.toggle('collapsed')">
        <div class="service-title">
          ...
          <h3>${mount}</h3>
          <span class="route-count">${routeCount} route${routeCount !== 1 ? "s" : ""}</span>
        </div>
        <div class="method-summary">
          ${Object.keys(methods).map((m) => methodBadge(m)).join("")}
        </div>
      </div>
...
export const createApiDocUi = (
  ...
  for (const [, methods] of Object.entries(doc)) {
    totalServices++;
  ...
  return (_req: any, res: any) => {
    ...
    const servicesHtml = Object.entries(doc)
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([mount, methods]) => renderService(mount, methods as any, t))
      .join("");

    const body = `
      <div class="page-header">
        <h1>${appName} — API Routes</h1>
        <div class="meta">${totalRoutes} routes across ${totalServices} services</div>
      </div>
      <input type="text" class="search-bar" placeholder="Search routes... (e.g. GET /users)" id="search"/>
      ${statsHtml}
      <div id="services">
        ${servicesHtml || '<div class="empty">No routes registered.</div>'}
      </div>
      <script>
