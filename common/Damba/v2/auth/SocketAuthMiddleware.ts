import { logWarn } from "@Damba/core/Logutils";
import { getTokenInfo, CheckTokenForDamba } from "./AuthMiddleware";

const CheckTokenForGoogleSocket = async (
  socket: any,
  token: string
): Promise<any> => {
  try {
...
const protectSocket = (public_key: string, jwt: any, roles?: string[]) => {
  logWarn("Protect socket");
  return async <S extends object = any>(
    socket: S & { data: { token: string; user: any; googleUser: any } },
    data?: any
  ): Promise<S | undefined> => {
    try {
      ...
      const authority: string[] = Array.isArray(payload?.authority)
        ? payload.authority
        : [];
